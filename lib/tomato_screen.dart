...
class TomatoScreen extends StatefulWidget {
  ...
  @override
  State<TomatoScreen> createState() => _TomatoLeafDiseaseDetectionScreenState();
}

class _TomatoLeafDiseaseDetectionScreenState extends State<TomatoScreen>
...
Future<void> _runModelOnImage(String imagePath) async {
  try {
    ...
    if (recognitions == null || recognitions.isEmpty) {
      ...
      // 🔍 Check for non-TomatoScreen plant diseases
      if (recognitionLabel != null && !recognitionLabel.startsWith("Tomato_")) {
        setState(() {
          label = "Wrong plant detected";
          confidence = (recognitionConfidence ?? 0.0) * 100;
        });
      } else {
        // ✅ TomatoScreen disease detected
        setState(() {
          label = recognitionLabel?.split('Tomato_').last.trim() ?? "Unknown disease";
          confidence = (recognitionConfidence ?? 0.0) * 100;
        });
      }
    }
