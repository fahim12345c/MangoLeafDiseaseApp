...
Future<void> _runModelOnImage(String imagePath) async {
  try {
    ...
    if (recognitions == null || recognitions.isEmpty) {
      ...
      // 🔍 Check for non-Mango plant diseases
      if (recognitionLabel != null && !recognitionLabel.startsWith("Mango_")) {
        setState(() {
          label = "Wrong plant detected";
          confidence = (recognitionConfidence ?? 0.0) * 100;
        });
      } else {
        // ✅ Mango disease detected
        setState(() {
          label = recognitionLabel?.split('Mango_').last.trim() ?? "Unknown disease";
          print("Detected disease: $label+${label.length}");
          confidence = (recognitionConfidence ?? 0.0) * 100;
        });
      }
    }
